...
- **Skill**（`skills/my-news/`）：Claude Code 的壳，把 CLI 的 JSON 整理成中文简报 / 列表 / 单条原文。

两个都装上，在 Claude Code 里说"看看今天的新闻"就行。
...
# Linux: sudo apt install newsboat && curl -LsSf https://astral.sh/uv/install.sh | sh

# 2. CLI
uv tool install git+https://github.com/KaleLjl/my-news.git

# 3. Skill（拷到 Claude Code 的 skills 目录）
git clone https://github.com/KaleLjl/my-news.git /tmp/my-news
mkdir -p ~/.claude/skills
cp -r /tmp/my-news/skills/my-news ~/.claude/skills/my-news
```
...
详细安装/排错见 [`skills/my-news/references/install.md`](skills/my-news/references/install.md)。
...
## 在 Claude Code 里用 Skill（推荐）

Skill 装好后，直接说人话就行：

| 你想干啥 | 说法 |
|---|---|
| 看今天有什么新的 | "看看今天的新闻" / "刷一下源" / `/my-news` |
| 罗列某个源的最近文章 | "列一下 simonw 最近 20 条" |
| 把某条原文给我 | "把 id 1042 的原文给我" / "总结一下那条" |
| 把某个 URL 展开 | "https://xxx 这篇讲了什么" |

Skill 自动判断走 `fetch` / `list` / `show`，简报会写一份到 `~/.local/share/my-news/digests/<时间戳>.md`，方便回看或被脚本推送。

---

## 直接用 CLI（不走 Claude Code 也行）

五个子命令，输出都是 UTF-8 JSON（`feeds` 是文本）。
...
想换地方：设 `MY_NEWS_CONFIG` / `MY_NEWS_DATA` 环境变量。
...
# 触发 Claude Code skill 生成简报到 digests/
